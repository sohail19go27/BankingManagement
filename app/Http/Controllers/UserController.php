...
class UserController extends Controller
{
    ...
    public function adminDashboard(){
        ...
        $data['bookingStats'] = [
            'today' => Bookings::whereDate('created_at', $today)->count(),
            'week' => Bookings::whereDate('created_at', '>=', $thisWeek)->count(),
            'cancelled' => Bookings::where('status', 0)->count(),
        ];

        $data['chartData'] = Bookings::selectRaw('DATE(created_at) as date, COUNT(*) as count')
